...
@app.route('/')
def index():
    """Página inicial que lista todos os serviços disponíveis."""
...
def get_available_times():
    ...
    for appt in appointments_on_date:
        ...
        end = start + timedelta(minutes=appt_service.duration)
    ...
    while potential_start_dt + service_duration <= work_end_dt:
        ...
        for busy_start, busy_end in busy_slots:
            busy_start_dt = datetime.combine(selected_date, busy_start)
            busy_end_dt = datetime.combine(selected_date, busy_end)
        ...
        # Avança para o próximo slot de verificação (a cada 15 minutos, por exemplo)
        potential_start_dt += timedelta(minutes=15)
